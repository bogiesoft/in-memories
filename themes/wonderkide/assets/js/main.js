...
$("document").ready(function($){
    ...
    //var check_double = 0;
    $("#update-drag-img").change(function(){
        var check = $("#update-drag-img-sortable > li").length;
        var data = [];
        for(i=0;i<check;i++){
            data[i] = document.getElementsByName("data-selected")[i].value;
        }

        updateSortGallery(data.join());
...
function updateZeny(id){
    if(confirm("ท่านต้องการจะรับเงินเลยใช่ไหม?")){
...
function updateSortGallery(data){
        var url = "/gallery/updatesort";
        $.ajax({
            type: "POST",
            url: url,
            data:"data="+data,
